function varargout = gaussian_copula_fit(family,u,varargin)
...
%   $Revision: 1.1.6.6 $  $Date: 2009/05/07 18:30:41 $
...
elseif ~all(all(0 < u & u < 1 | isnan(u)))
    error('stats:copulafit:DataOutOfRange', ...
          'U must contain values strictly between 0 and 1 or NaN.');
end
...
[eid,errmsg,alpha,method,options] = getargs(pnames, dflts, varargin{:});
if ~isempty(eid)
...
if nu < 1000
    ...
    if any(t)
        ...
        oneminusz(t) = betaincinv(2.*abs(q(t)),0.5,nu/2);
        z(t) = 1 - oneminusz(t);
    end
    if any(~t)
        z(~t) = betaincinv(1-2.*abs(q(~t)),nu/2,0.5);
        oneminusz(~t) = 1 - z(~t);
    end
